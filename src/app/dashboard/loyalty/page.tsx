...
export default function LoyaltyPage() {
    ...
    const fetchData = useCallback(async (page = 1) => {
        ...
        try {
            ...
            setRanking(rankingRes.data.data || []); // <-- CORRECCIÓN
            setTotalPages(rankingRes.data.totalPages || 1); // <-- CORRECCIÓN
            setCurrentPage(rankingRes.data.currentPage || 1); // <-- CORRECCIÓN
            setPerfectAttendance(perfectAttendanceRes.data || []); // <-- CORRECCIÓN
        } catch (error) {
            console.error(error);
            setRanking([]);
