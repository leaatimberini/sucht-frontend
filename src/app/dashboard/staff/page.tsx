...
export default function StaffPage() {
  ...
  const limit = 10; // Miembros del staff por página

  const fetchStaff = useCallback(async (page = 1) => {
    setIsLoading(true);
    try {
      // 1. Añadimos los parámetros de paginación a la llamada
      const response = await api.get('/users/staff', {
        params: { page, limit }
      });
      
      // 2. Leemos la respuesta paginada correctamente
  ...
  return (
    <div>
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        {/* 3. AÑADIMOS CONTROLES DE PAGINACIÓN */}
