...
// --- SUB-COMPONENTE: TABLA DE RESUMEN ---
function BirthdaySummaryTable({ data, onEdit }: { data: BirthdaySummary[], onEdit: (summary: BirthdaySummary) => void }) {
  if (data.length === 0) {
    return <p className="text-zinc-500 text-center py-10">No se reclamaron beneficios de cumpleaños para este evento.</p>;
  }
  return (
        ...
        <tbody>
          {data.map((summary) => (
            <tr key={summary.user.id} className="border-b border-zinc-800 last:border-b-0">
              ...
              <td className="p-4">
                <span className={`px-2 py-1 text-xs font-semibold rounded-full ${summary.reward.isGiftClaimed ? 'bg-zinc-500/20 text-zinc-400' : 'bg-green-500/20 text-green-400'}`}>
                  {summary.reward.isGiftClaimed ? 'Canjeado' : 'Pendiente'}
                </span>
              </td>
              <td className="p-4">
                <button onClick={() => onEdit(summary)} className="text-pink-400 hover:text-pink-300 flex items-center gap-2"><Edit size={16} /> Editar</button>
              </td>
            </tr>
          ))}
        </tbody>
...
export default function AdminBirthdayPage() {
  ...
  useEffect(() => {
    // Cargar la lista de eventos para el selector
  ...
  return (
    <AuthCheck allowedRoles={[UserRole.ADMIN, UserRole.OWNER]}>
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold text-white mb-6">Birthday Management</h1>
        ...
        {/* Tabla de Resultados */}
        {isLoading 
          ? <div className="flex justify-center items-center py-20"><Loader2 className="animate-spin text-pink-500" size={32} /></div>
          : selectedEventId && <BirthdaySummaryTable data={summaryData} onEdit={setEditingSummary} />
        }
