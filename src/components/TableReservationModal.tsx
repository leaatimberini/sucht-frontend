...
export function TableReservationModal({ eventId, onClose }: { eventId: string; onClose: () => void; }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                setVipTiers(tiersRes.data.filter((t: TicketTier) => t.productType === 'vip_table'));
            } catch (error) {
    ...
    const handleConfirmReservation = async (paymentType: 'full' | 'partial') => {
        ...
        const tier = vipTiers.find(t => t.name.toLowerCase().includes(selectedTable.category.name.toLowerCase()));
        if (!tier) {
            toast.error("No se encontró un producto de venta para esta mesa.");
            return;
        }

        try {
            const payload = { eventId, ticketTierId: tier.id, quantity: 1, paymentType };
    ...
    const tierForSelectedTable = selectedTable ? vipTiers.find(t => t.name.toLowerCase().includes(selectedTable.category.name.toLowerCase())) : null;
    ...
    return (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4 animate-fade-in">
            ...
            {selectedTable && tierForSelectedTable && (
                <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 animate-fade-in">
                    <div className="bg-zinc-800 rounded-lg p-6 max-w-sm w-full space-y-4">
                        <h3 className="text-xl font-bold text-white">Confirmar Mesa {selectedTable.tableNumber}</h3>
                        <p className="text-zinc-300">Estás por reservar la <span className="font-bold">{tierForSelectedTable.name}</span>.</p>
                        <p className="text-lg font-bold text-pink-400">Precio Total: ${tierForSelectedTable.price}</p>
                        <div className="space-y-2">
                            ...
                            {tierForSelectedTable.allowPartialPayment && (
                                <button onClick={() => handleConfirmReservation('partial')} className="w-full bg-zinc-600 hover:bg-zinc-700 text-white font-bold py-2 px-4 rounded-lg">Pagar Seña (${tierForSelectedTable.partialPaymentPrice})</button>
                            )}
                        </div>
                        <button onClick={() => setSelectedTable(null)} className="w-full text-zinc-400 mt-2 text-sm">Cancelar</button>
                    </div>
                </div>
            )}
        </div>
