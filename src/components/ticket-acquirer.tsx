...
export function TicketAcquirer({ eventId }: { eventId: string }) {
  ...
  const handleAcquire = async () => {
    ...
    setIsLoading(true);
    try {
      await api.post('/tickets/acquire', {
        eventId,
  ...
  if (tiers.length === 0) {
    return <p className="text-zinc-400">No hay entradas disponibles para este evento.</p>;
  }
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-6 space-y-4">
      ...
      <button 
        ...
        disabled={isLoading}
        className="w-full bg-pink-600 hover:bg-pink-700 text-white font-bold py-3 rounded-lg disabled:opacity-50"
      >
