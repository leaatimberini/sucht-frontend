...
import { LogIn, User, ChevronDown, LogOut, LayoutGrid, QrCode, BarChartHorizontal, GlassWater } from 'lucide-react'; // 1. Ícono añadido
...
  const isVerifier = user?.roles.includes(UserRole.VERIFIER);
  const isBarra = user?.roles.includes(UserRole.BARRA); // 2. Flag para el nuevo rol
...
                  )}
                  ...
                  {/* --- 3. NUEVO ENLACE PARA EL ROL BARRA --- */}
                  {(isBarra || isAdmin) && (
